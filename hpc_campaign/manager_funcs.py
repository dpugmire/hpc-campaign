...
GAUSSIAN_SPLAT_KIND = "gaussianSplat"
SUPPORTED_VISUALIZATION_ITEM_FORMATS = {"IMAGE": "IMAGE"}
...
def ensure_gaussian_splat_tables(cur: sqlite3.Cursor, con: sqlite3.Connection):
    sql_execute(
        cur,
        "create table if not exists gaussian_splat" + "(datasetid INT PRIMARY KEY, metadata TEXT)",
    )
    sql_commit(con)
...
def add_gaussian_splat_metadata_to_archive(
    datasetid: int,
    metadata: dict,
    cur: sqlite3.Cursor,
    indent: str = "",
    ...
    if verbose:
        print(f"{indent}Add Gaussian-splat metadata for dataset {datasetid} to archive")
    cur_ds = sql_execute(
        cur,
        "insert into gaussian_splat (datasetid, metadata) values (?, ?) "
        "on conflict (datasetid) do update set metadata = excluded.metadata returning rowid",
        (datasetid, json.dumps(metadata, sort_keys=True)),
    )
    return int(cur_ds.fetchone()[0])
...
def process_gaussian_splat_data(
    args: argparse.Namespace,
    cur: sqlite3.Cursor,
    host_id: int,
    dir_id: int,
    ...
    payload = bytes(args.gaussian_splat_data)
    metadata = dict(getattr(args, "gaussian_splat_metadata", {}) or {})
    if not payload:
        raise ValueError("gaussian_splat_data requires a non-empty payload")
    if metadata.get("kind") != GAUSSIAN_SPLAT_KIND:
        raise ValueError(f"gaussian_splat_data metadata.kind must be {GAUSSIAN_SPLAT_KIND!r}")
    if str(metadata.get("encoding", "") or "").lower() != "raw":
        raise ValueError("Only raw Gaussian-splat encoding is supported currently")
    if str(metadata.get("compression", "") or "").lower() != "none":
        raise ValueError("Only uncompressed Gaussian-splat payloads are supported currently")
    if str(metadata.get("dtype", "") or "").lower() != "float32":
        raise ValueError("Only float32 Gaussian-splat payloads are supported currently")
    if int(metadata.get("spatial_dimensions", 0)) != 2:
        raise ValueError("gaussian_splat_data requires spatial_dimensions=2")
    count = int(metadata.get("count", 0))
    if count <= 0:
        raise ValueError("gaussian_splat_data metadata.count must be positive")
    expected_bytes = (6 * count + 1) * 4
    if len(payload) != expected_bytes:
        raise ValueError(f"Gaussian-splat payload has {len(payload)} bytes; expected {expected_bytes}")
    ...
    if args.name is not None:
        ...
        checksum = sha1(payload + json.dumps(metadata, sort_keys=True).encode("utf-8")).hexdigest()
        unique_id = uuid.uuid5(uuid.NAMESPACE_OID, checksum).hex
        dataset = f"memory-gaussian-splats/splat-{unique_id[:12]}.raw"

    replica_name = getattr(args, "replica_name", "") or join("memory-gaussian-splats", f"{unique_id}.raw")
    mt = time_ns()
    verbose = is_verbose(args)
    if verbose:
        print(f"Process in-memory Gaussian splat {dataset}")

    ds_id = add_dataset_to_archive(dataset, cur, unique_id, GAUSSIAN_SPLAT_FORMAT, mt, indent="  ", verbose=verbose)
    rep_id = add_replica_to_archive(
        host_id,
        dir_id,
        0,
    ...
    add_gaussian_splat_metadata_to_archive(ds_id, metadata, cur, indent="  ", verbose=verbose)
    add_file_to_archive(
        args,
        "",
        cur,
...
def add_gaussian_splat_data(args: argparse.Namespace, cur: sqlite3.Cursor, con: sqlite3.Connection):
    ensure_gaussian_splat_tables(cur, con)
    long_host_name, short_host_name = get_host_name(args)
    verbose = is_verbose(args)
    ...
    process_gaussian_splat_data(args, cur, host_id, dir_id, key_id, long_host_name + rootdir, rootdir)
    sql_commit(con)
